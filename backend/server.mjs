...
app.post("/api/generate-site", async (req, res) => {
  try {
    ...
    const result = await genAI.models.generateContent({
      ...
      contents: `Ты — ИИ-разработчик. Сгенерируй ПОЛНЫЙ HTML-документ с CSS и JS внутри. Только HTML код без лишнего текста. Используй случайную цветовую схему и уникальный дизайн — каждый раз разный стиль. Например: минимализм, glassmorphism, неон, градиент, природные тона, космос, ретро и т.д.. Там где нужны картинки используй тег <img> с классом "ai-image" и data-prompt атрибутом описывающим что должно быть на картинке на английском. НЕ используй placeholder.com или любые другие placeholder сервисы. Оставь src пустым. Описание сайта: ${prompt}`
    });
